...
-- =========================================================================
-- Añade:
--   - Trigger que detecta 2 match_reports y compara: coincidentes → validated,
--     divergentes → disputed. Inserta los sets cuando validated.
--   - Función helper score_normalize(jsonb) que devuelve estructura canónica.
--   - Función helper score_winner(score_json, pair_a_id, pair_b_id) → pair_id.
--   - Función admin_resolve_dispute(match_id, score_json, admin_player_id)
--     que finaliza un partido en `disputed`.
-- =========================================================================

set search_path = public;

-- -------------------------------------------------------------------------
-- Helper: normaliza un score_json a array ordenado por set asc.
-- score_json esperado: [{"set": 1, "a": 6, "b": 4}, ...]
-- Devuelve un texto canónico tipo "6-4,4-6,6-4" para comparar.
-- =========================================================================
...
-- -------------------------------------------------------------------------
-- Helper: determina el winner_pair_id según el score y los IDs de las pairs.
-- Reglas (§8):
--   - 2 sets ganados (de hasta 3) → ganador del partido.
--   - Empate a 1 → tercer set decisivo al 6, TB a 7 si 6-6.
-- =========================================================================
create or replace function score_winner(
  p_score jsonb,
  p_pair_a_id uuid,
  p_pair_b_id uuid
)
...
-- -------------------------------------------------------------------------
-- Helper: inserta los sets validados a partir del score_json.
-- =========================================================================
...
-- -------------------------------------------------------------------------
-- Trigger principal: tras insert/update en match_reports, compara y actualiza
-- matches.status.
-- Reglas:
--   - Si el report es de un admin (reporter_pair_side='admin'), aplica
--     inmediatamente: status='validated', insertar sets, set winner_pair_id.
--     Cualquier otro report previo queda obsoleto pero conservado.
--   - Si hay 2 reports de capitanes (uno side='a', otro side='b'):
--     · coincidentes (score_canonical igual) → status='validated', insertar
--       sets, set winner_pair_id.
--     · divergentes → status='disputed'.
--   - Si hay sólo 1 report de capitán → status='pending_validation'.
-- =========================================================================
...
-- -------------------------------------------------------------------------
-- Función pública: submit_match_report (server action wrapper)
-- Permite al capitán autenticado registrar su report con seguridad.
-- =========================================================================
...
begin
  ...
  if v_player_id is null and not v_is_admin then
  ...
  if v_is_admin then
    v_side := 'admin';
    -- Si el admin no tiene player profile, usamos su user_id como reporter (no recomendado).
    -- Mejor: requerir que el admin tenga su player_id real.
    if v_player_id is null then
      raise exception 'admin_without_player_profile';
    end if;
  else
    -- Determinar el side: ¿captain de pair_a o pair_b?
  ...
  -- Upsert por (match_id, reporter_player_id)
